...
local function onvif_cmd(sendurl, command, sendbody, authheader)
  ...
  if sendbody then
    ...
    if (command == 'GetSystemDateAndTime') or
       ...
       (command == 'GetCapabilities') or
       (command == 'GetServices') then
...
local function create_authdata_table(authrecord)
  ...
  authdata.domain = parms:match('domain="([^"]+)"')               -- ** not handled in current code **
    
  --[[
  if parms:match('opaque=') then                                  -- should have quotes around value
    local opaque_val = parms:match('opaque="([^"]+)"')
    if opaque_val == nil then                                       -- store opaque value WITH quotes 
      authdata.opaque = '""'                                        -- handle opaque="" case
    else
      authdata.opaque = '"' .. opaque_val .. '"'
    end
  end
      
  if parms:match('domain=') then                                  -- ** not handled in current code **
    local domain_val = parms:match('domain="([^"]+)"')
    if domain_val == nil then
      authdata.domain = '""'
    else
      authdata.domain = '"' .. domain_val .. '"'
    end
  end
  --]]
      ...
      elseif code == 401 then         -- HTTP authorization is required
        ...
        if auth_record then
          ...
          if auth_record:find('gSOAP Web Service', 1, 'plaintext') then
            ...
            log.debug ('auth_record: ', auth_record)
...
local function GetServices(device, device_serviceURI)
...
<s:Envelope xmlns:s="http://www.w3.org/2003/05/soap-envelope">
  ...
  if xml_body then
    ...
    if common.is_element(xml_body, {'GetServicesResponse'}) then
      
        return xml_body['GetServicesResponse']
        
    else
      log.error ('Missing Services list from', device.label)
    end
  end
...
<s:Envelope
  ...
  local request_part1b_V1 = [[</wsa:Address>
      </wsnt:ConsumerReference>
]]


  local request_part1a_V2 = [[
<s:Envelope
    xmlns:s="http://www.w3.org/2003/05/soap-envelope"
    xmlns:wsa="http://www.w3.org/2005/08/addressing"
    xmlns:tet="http://www.onvif.org/ver10/events/wsdl"
  ...
  local request_part1b_V2 = [[</wsa:Address>
      </ConsumerReference>
]]
...
--]]
  ...
  local request = request_part1a_V2 .. listenURI .. request_part1b_V2 .. request_lastpart_V2
  --local request
